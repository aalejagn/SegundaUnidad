...
def ventana_login(ventana):
    ...
    Button(marco_login, text="Ingresar", font=("Arial", 13), width=15, 
           command=lambda: validar_usuarios(tipo_usuario.get(), entry_contraseña.get(), ventana, marco_login))\
        .grid(row=5, column=0, columnspan=2, pady=20)

"""
Creamos la funcion de validar datos para usuarios
"""
def validar_usuarios(tipo_usuario, contraseña, ventana, marco_login):
...
"""
def barra_lateral(ventana, tipo_usuario):  # Removed unused marco_login parameter
    ...
    funciones = {
        "Clientes": lambda: manejo_usuarios(ventana, tipo_usuario, barra_lateral),  # Fixed rol.get() to tipo_usuario
    ...
    Button(main_frame, text="¡COMENZAR AHORA!", font=("Arial", 14, "bold"), bg="#FFA500", fg="white",
           command=lambda: manejo_usuarios(ventana, tipo_usuario, barra_lateral)).pack(pady=30)  # Fixed rol.get()
